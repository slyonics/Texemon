...
namespace Panderling.GameObjects.Maps
{
    ...
    {
        public const int NODE_SIZE = Tile.TILE_SIZE / 2;
        ...
        public NavNode(Map iMap, int x, int y)
        {
            nodeX = x;
            nodeY = y;
        ...
        public void AssignNeighbors(Map map)
        {
            if (nodeX > 0) neighborList.Add(map.GetNavNode(nodeX - 1, nodeY));
            if (nodeY > 0) neighborList.Add(map.GetNavNode(nodeX, nodeY - 1));
            if (nodeY < map.MapHeight * 2 - 2) neighborList.Add(map.GetNavNode(nodeX, nodeY + 1));
            if (nodeX < map.MapWidth * 2 - 2) neighborList.Add(map.GetNavNode(nodeX + 1, nodeY));

            if (nodeX > 0 && nodeY > 0) neighborList.Add(map.GetNavNode(nodeX - 1, nodeY - 1));
            if (nodeX > 0 && nodeY < map.MapHeight * 2 - 2) neighborList.Add(map.GetNavNode(nodeX - 1, nodeY + 1));
            if (nodeX < map.MapWidth * 2 - 2 && nodeY > 0) neighborList.Add(map.GetNavNode(nodeX + 1, nodeY - 1));
            if (nodeX < map.MapWidth * 2 - 2 && nodeY < map.MapHeight * 2 - 2) neighborList.Add(map.GetNavNode(nodeX + 1, nodeY + 1));
        }
        ...
        public bool FitsActor(Actor actor)
        {
            ...
            int tileStartX = boundsForActor.Left / Tile.TILE_SIZE;
            int tileEndX = boundsForActor.Right / Tile.TILE_SIZE;
            int tileStartY = boundsForActor.Top / Tile.TILE_SIZE;
            int tileEndY = boundsForActor.Bottom / Tile.TILE_SIZE;
